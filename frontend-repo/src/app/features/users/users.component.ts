...
export class UsersComponent implements OnInit {
  ...
  loadUsers(): void {
    this.api.nextLevelUsers().subscribe((res: any) => {
      ...
      if (!this.users.find((u) => u._id === this.receiverId)) this.receiverId = '';
      if (!this.users.find((u) => u._id === this.editUserId)) this.editUserId = '';
    });
  }
  ...
  updateUserPassword(): void {
    ...
    this.api.changeUserPassword(this.editUserId, { newPassword: this.newPassword }).subscribe({
      next: () => {
  ...
  get hierarchyTargets(): any[] {
    return this.flattenTree(this.hierarchyTree).filter((u) => !!u.parentId);
  }
  creditInHierarchy(): void {
    ...
    this.api.adminCredit(this.adminCreditTargetId, { amount: Number(this.adminCreditAmount) }).subscribe({
      next: (res) => {
      ...
      error: (err) => {
        this.adminCreditError = err?.error?.message || 'Unable to credit hierarchy user.';
      }
    });
  }
