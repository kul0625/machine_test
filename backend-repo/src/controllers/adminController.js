...
const hierarchy = async (req, res) => {
  ...
  const allowed =
    req.auth.role === "OWNER" ||
    req.auth.role === "ADMIN" ||
    (await isDescendant(req.auth.sub, target)) ||
    req.auth.sub === target;
...
const creditAny = async (req, res) => {
  ...
  req.app.get("io").emit("balance-updated", {
    ...
    balances: {
      [parent._id]: result.sender.balance,
      [user._id]: result.receiver.balance,
    },
  });

  res.json({
    message: "Admin credit successful",
    parentDebited: parent.username,
    commission: result.commission,
  });
};

const balanceSummary = async (_req, res) => {
  const users = await User.find({ role: { $nin: ["ADMIN"] } }).select(
    "username name level role balance commissionEarned parentId",
  );
